...
public class Block {
    ...
    public Block(Vector3 offset, MeshUtils.BlockType type, Chunk chunk)
    {
        ...
        MeshUtils.BlockType topType = type;
        MeshUtils.BlockType bottomType = type;

        if(type == MeshUtils.BlockType.GRASSSIDE)
        {
            topType = MeshUtils.BlockType.GRASSTOP;
            bottomType = MeshUtils.BlockType.DIRT;
        }
        ...
        if (type != MeshUtils.BlockType.AIR)
        {
            ...
            if (!HasSolidNeighbour((int)blockLocalPos.x, (int)blockLocalPos.y - 1, (int)blockLocalPos.z))
                quads.Add(new Quad(MeshUtils.BlockSide.BOTTOM, offset, bottomType));

            if (!HasSolidNeighbour((int)blockLocalPos.x, (int)blockLocalPos.y + 1, (int)blockLocalPos.z))
                quads.Add(new Quad(MeshUtils.BlockSide.TOP, offset, topType));
